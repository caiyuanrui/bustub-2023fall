...
auto Optimizer::OptimizeSortLimitAsTopN(const AbstractPlanNodeRef& plan)
  ...
  if (optimized_plan->GetType() == PlanType::Sort &&
      optimized_plan->GetChildAt(0)->GetType() == PlanType::Limit) {
    auto sort_node = dynamic_cast<const SortPlanNode&>(*optimized_plan);
    auto limit_node =
        dynamic_cast<const LimitPlanNode&>(*optimized_plan->GetChildAt(0));
    return std::make_shared<const TopNPlanNode>(
        optimized_plan->output_schema_, limit_node.GetChildAt(0),
  ...
  if (optimized_plan->GetType() == PlanType::Limit &&
    ...
    auto sort_node =
        dynamic_cast<const SortPlanNode&>(*optimized_plan->GetChildAt(0));
    return std::make_shared<const TopNPlanNode>(
        optimized_plan->output_schema_, sort_node.GetChildAt(0),
        sort_node.GetOrderBy(), limit_node.GetLimit());
  }
